...
; 字符串
PMMessage:		db	"Now in protect mode", 0	; 进入保护模式后显示此字符串
...
LABEL_SEG_CODE32:
	...
	cld
.1:
	lodsb  ;从esi指向的源地址中逐一读取一个字符,送入AL中; (然后,可以先判断这个字符是什么字符,如0dh,0ah 之类等,再执行相应的操作); 
	test	al, al
	jz	.2
	mov	[gs:edi], ax
	add	edi, 2
...
;	mov	[gs:edi], ax
	...
	; 下面显示姓名
	mov	ah, 0Ch			; 0000: 黑底    1100: 红字
	mov	esi, OffsetprintMessage1 ; 源数据偏移
...
LABEL_CODE_B:
	...
	; 下面显示学号
	mov	ah, 0Ch			; 0000: 黑底    1100: 红字
	mov	esi, OffsetprintMessage2; 源数据偏移
	mov	edi, (80 * 14 + 0) * 2	; 目的数据偏移。屏幕第 14 行, 第 0 列。
	cld
.5:
	lodsb  ;从esi指向的源地址中逐一读取一个字符,送入AL中; (然后,可以先判断这个字符是什么字符,如0dh,0ah 之类等,再执行相应的操作); 
	test	al, al
